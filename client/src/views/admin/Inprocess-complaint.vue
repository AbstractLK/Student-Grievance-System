  ...
  methods: {
    ...
    async fetchNotProcessedComplaints() {
      try {
        ...
        const response = await axios.get(URL+'/complaint/inProcessedComplaint', {
          headers:{
  ...
  <v-card flat>
    ...
    <v-data-table :headers="headers" :items="filteredComplaints" :search="search" class="mx-5" height="430px"
      ...
      <template v-slot:item.status="{ item }">
        <v-btn size="small" style="width: 90px; font-size: 10px;" color="orange-lighten-1" variant="flat">{{ item.status
          }}</v-btn>
      </template>
